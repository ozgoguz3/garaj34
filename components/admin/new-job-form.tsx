...
import { CheckCircle2, ExternalLink, ShieldCheck, AlertTriangle } from 'lucide-react'
...
export function NewJobForm({ organizationId, businessSlug }: { organizationId: string, businessSlug: string }) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    startTransition(async () => {
      ...
      setCustomerName(''); setPlate(''); setCarModel(''); setPhone(''); setPrice('')
    ...
    <SectionCard title="Yeni İşlem Ekle" description="Plaka girin, hizmetleri seçin ve tek tıkla sisteme kaydedin.">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="flex flex-col gap-2">
            <Label>Tutar (₺) <span className="text-muted-foreground text-xs">(İsteğe bağlı)</span></Label>
            <Input inputMode="decimal" value={price} onChange={(e) => setPrice(e.target.value.replace(/[^\d.,]/g, ''))} placeholder="Örn: 1500" className="h-11 font-mono" />
          </div>
        </div>
            ...
            {SERVICE_OPTIONS.map((service) => (
              <button key={service} type="button" onClick={() => toggleService(service)}
                className={`rounded-full border px-4 py-2 text-sm transition-all ${
                  services.includes(service) ? 'border-neon bg-neon/10 text-neon shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'text-muted-foreground'
                }`}>{service}</button>
            ))}
          </div>
        </div>
            ...
            <div className="flex gap-2">
              {WARRANTY_PRESETS.map((p) => (
                <button key={p.months} type="button" onClick={() => setWarrantyMonths(p.months)}
                  className={`rounded-full border px-4 py-1.5 text-xs transition-all ${warrantyMonths === p.months ? 'border-amber-400 bg-amber-500/15 text-amber-400' : 'text-muted-foreground'}`}>{p.label}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {formError && (
          <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-300">{formError}</p>
        )}
