...
export function BrandingForm({ business, slug }: BrandingFormProps) {
  ...
  function handleSave() {
    startTransition(async () => {
      await updateBrandingAction(slug, business.id, { logoUrl, tagline, primaryColor: color })
      setSaved(true)
      router.refresh()
  ...
  return (
    <SectionCard
      ...
      description="Logo, slogan ve ana renk — hem admin panelinde hem müşteri ekranında kullanılır."
    >
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            Logo dosyanı bir görsel barındırma sitesine (örn. imgur.com) yükleyip linkini buraya yapıştırabilirsin.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Input
            ...
            placeholder="Premium Oto Detaylama"
            className="h-11 bg-input/40"
          />
        </div>
            ...
            {COLOR_PRESETS.map((preset) => (
              <button
                ...
                className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium transition-all ${
                  color === preset.value ? 'border-foreground/60 ring-2 ring-foreground/20' : 'border-border'
                }`}
              >
        ...
        <Button
          onClick={handleSave}
          disabled={isPending}
          className="h-12 w-full text-neon-foreground"
          style={{ backgroundColor: color }}
        >
          {saved ? (
            <><CheckCircle2 className="mr-2 size-4" /> Kaydedildi</>
          ) : isPending ? (
            'Kaydediliyor...'
          ) : (
