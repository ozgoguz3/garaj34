...
export function JobCard({ visit, organizationId, businessSlug }: { visit: Visit; organizationId: string; businessSlug: string }) {
  ...
  function apply(next: VisitStatus) {
    ...
    const prev = status
    setStatus(next) // anında görsel geri bildirim — DB beklenmez
    startTransition(async () => {
      try {
        await updateVisitStatusAction(businessSlug, organizationId, visit.id, next)
      } catch {
        setStatus(prev) // hata olursa geri al
      } finally {
        router.refresh()
      }
    })
  ...
  return (
    <article className={cn('flex flex-col gap-4 rounded-xl border bg-background/40 p-4 transition-all duration-200', isReady ? 'border-neon/40 glow-neon' : 'border-border', isPending && 'opacity-80')}>
        ...
        <div className="flex min-w-0 flex-col gap-1">
          ...
          {visit.services && (
            <div className="mt-1 flex flex-wrap gap-1">
              {visit.services.map((s, i) => <span key={i} className="rounded-md bg-secondary/60 px-1.5 py-0.5 text-[10px]">{s}</span>)}
            </div>
          )}
        </div>
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <button
              ...
              disabled={isPending}
              onClick={() => apply(s.key)}
              className={cn('flex h-9 items-center justify-center rounded-lg border text-[11px] font-bold transition-all duration-150 active:scale-[0.97]', active && 'glow-cyan border-cyan bg-cyan/15 text-cyan', done && 'border-neon/30 bg-neon/10 text-neon', !active && !done && 'border-border bg-muted/60 text-muted-foreground')}
            >
              {s.adminLabel}
            </button>
          )
      ...
      <div className="flex flex-col gap-1.5 sm:flex-row">
        ...
        <Button variant="ghost" className="h-9 text-xs text-muted-foreground" render={<a href={`/${businessSlug}/${plateToSlug(displayPlate)}`} target="_blank" rel="noopener noreferrer" />}>
          <ExternalLink className="mr-1.5 size-3.5" /> Takip Sayfası
        </Button>
      </div>
    </article>
  )
