...
const SEGMENTS: { value: CampaignSegment; label: string; description: string; icon: React.ElementType }[] = [
  { 
    value: 'inactive_30', 
    label: '30-60 Gün Gelmeyenler', 
    description: 'Yakında unutmaya başlayacaklar – hatırlatma için ideal.',
    icon: Clock
  },
  { 
    value: 'inactive_60', 
    label: '60+ Gün Gelmeyenler', 
    description: 'Uzun süredir uğramayan, kaybetme riski olan müşteriler.',
    icon: TrendingUp
  },
  { 
    value: 'ceramic_ppf_only', 
    label: 'Seramik & Film Yaptıranlar', 
    description: 'Garantili işlemler – periyodik bakım veya yenileme satmak için.',
    icon: Sparkles
  },
  { 
    value: 'high_value', 
    label: 'En Değerli 20 Müşteri', 
    description: 'Toplam harcamaya göre en çok kazandıran müşteriler.',
    icon: Zap
  },
  { 
    value: 'all_customers', 
    label: 'Tüm Müşteriler', 
    description: 'Genel bir duyuru/kampanya için tüm müşteri listesi.',
    icon: Users
  },
]
...
const MESSAGE_TEMPLATES = [
  {
    ...
    text: 'Merhaba {isim}! 🌸 Bahar geldi, aracınız için detaylı bakım zamanı! %20 indirim fırsatını kaçırmayın. Randevu için bize yazın.',
  },
  {
    name: 'Yaz Hazırlığı',
    text: 'Sayın {isim}, yaz gelmeden seramik kaplama ile aracınızı güneşten koruyun! ☀️ Sınırlı sayıda slot kaldı.',
  },
  {
    name: 'Garanti Hatırlatma',
    text: 'Merhaba {isim}! Seramik kaplama garantiniz yakında sona eriyor. Ücretsiz kontrol için hemen randevu alın!',
  },
  {
    name: 'Sadakat Tebrik',
    text: 'Sayın {isim}, bizi tercih ettiğiniz için teşekkürler! 💙 Sadık müşterilerimize özel %15 indirim sizin için hazır.',
  },
]
...
export function CampaignBuilder({ businessId, businessSlug, initialData = [] }: CampaignBuilderProps) {
  ...
  function sendTo(target: CampaignTarget) {
    const isUnknown = !target.customerName || 
      target.customerName.toLowerCase().includes('isimsiz') || 
      target.customerName.toLowerCase().includes('iİsimsiz')
  ...
  function sendToAll() {
    targets.forEach((target, index) => {
      setTimeout(() => sendTo(target), index * 1000) // Her saniye 1 mesaj
    })
  }
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      segment === s.value 
                        ? 'border-neon bg-neon/10 shadow-[0_0_15px_rgba(var(--neon-rgb),0.2)] scale-[1.02]' 
                        : 'border-border hover:bg-muted/40 hover:border-neon/30 hover:scale-[1.01]',
                    )}
                  >
                    <div className="flex items-center gap-2">
                      <Icon className={cn('size-4', segment === s.value && 'text-neon')} />
                      <span className={cn('text-sm font-semibold', segment === s.value && 'text-neon')}>
                        {s.label}
                      </span>
                    </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className={cn(copiedTemplate && 'text-neon')}
                >
          ...
          <div className="flex items-center gap-3">
            ...
            {targets.length > 0 && (
              <>
                ...
                >
                  <Download className="mr-1 size-3" /> CSV İndir
                </Button>
                <Button
                  variant="outline"
                  size="xs"
                ...
                >
                  <Megaphone className="mr-1 size-3" /> Tümüne Gönder
                </Button>
              </>
            )}
          </div>
        ...
        {isPending ? (
          ...
          <ul className="flex flex-col divide-y divide-border/50">
            {targets.map((t) => {
              ...
              return (
                <li key={t.plate} className="flex items-center justify-between gap-3 py-3.5 first:pt-1 last:pb-1 hover:bg-muted/20 transition-colors rounded-lg px-2">
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="font-mono text-[11.5px] text-muted-foreground flex items-center gap-1.5">
                      ...
                      {t.phone}
                      <span className="text-amber-400">• {daysSince} gün önce</span>
                    </span>
                  </div>
                  <Button
                    size="sm"
                    ...
                    className={cn(
                      ...
                      !isSent && 'border-neon/40 text-neon hover:bg-neon/15 hover:text-neon shadow-sm hover:shadow-neon/20',
