    ...
    <>
        ...
        <div className="no-scrollbar flex items-center gap-2 overflow-x-auto">
          ...
          {visits.map((v) => (
            <button
              ...
              aria-label={`${v.plate} müşteri ekranını önizle`}
            >
      ...
      {preview && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-background/80 sm:items-center sm:p-4" onClick={() => setPreview(null)}>
          <div
            className="relative flex max-h-[92dvh] w-full max-w-md flex-col overflow-hidden rounded-t-2xl border border-border bg-background sm:rounded-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="glass-blur flex items-center justify-between border-b border-border px-4 py-3">
              <span className="flex items-center gap-2 text-sm text-muted-foreground">
                <Eye className="size-4 text-cyan" /> Müşteri ne görüyor?
              </span>
              <Button variant="ghost" size="icon-sm" onClick={() => setPreview(null)}><X className="size-5" /></Button>
            </div>
            <div className="overflow-y-auto">
              <TrackingView visit={preview} businessName={businessName} />
            </div>
          </div>
        </div>
      )}
