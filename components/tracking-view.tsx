...
export function TrackingView({
  ...
  const displayPlate = visit.plate || 'BİLİNİYOR'
  ...
  return (
    ...
    >
      <div aria-hidden className="pointer-events-none absolute inset-x-0 top-0 h-72 bg-[radial-gradient(ellipse_at_top,rgba(16,185,129,0.18),transparent_65%)]" />
      ...
      <div className="relative mx-auto flex w-full max-w-md flex-col gap-8 px-5 pt-8">
        <header className="flex flex-col items-center gap-3 text-center">
          ...
          <Badge className="glow-neon gap-1.5 border-neon/40 bg-neon/15 px-3 py-1 text-neon">
            <Sparkles className="size-3.5" /> Canlı Takip
          </Badge>
          <h1 className="text-balance text-2xl font-bold tracking-tight">{businessName}</h1>
          <p className="text-sm text-muted-foreground">Sayın {firstName}, aracınız güvenli ellerde.</p>
        </header>

        <section className="flex flex-col items-center gap-4">
          <LicensePlate plate={displayPlate} size="lg" className="shadow-2xl" />
          {visit.carModel && <div className="font-semibold text-lg text-white/90">{visit.carModel}</div>}
          {visit.services && visit.services.length > 0 && (
            <div className="flex flex-wrap items-center justify-center gap-1.5 pt-1">
              {visit.services.map((s, i) => (
                <span key={i} className="flex items-center gap-1 rounded-full border border-neon/30 bg-neon/10 px-3 py-1 text-xs font-medium text-neon">
        ...
        <section className="glass rounded-2xl p-6">
          <div className="mb-6 flex items-center justify-between">
            <h2 className="text-sm font-semibold uppercase tracking-widest text-muted-foreground">İşlem Adımları</h2>
            <span className="font-mono text-xs text-neon">{Math.min(currentIndex + 1, STEPS.length)}/{STEPS.length}</span>
          </div>
          <StatusTracker status={visit.status} />
        </section>

        {isCompleted && googleMapsUrl && (
          <section className="flex flex-col items-center gap-5 pt-4 text-center animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="flex flex-col gap-1">
              <h2 className="text-lg font-semibold">Aracınız Teslimata Hazır!</h2>
              <p className="text-sm text-muted-foreground">Hizmetimizden memnun kaldıysanız bize 5 yıldız vererek destek olabilirsiniz.</p>
            </div>
            <Button
              size="lg"
