...
export type StepConfig = { key: VisitStatus; adminLabel: string; title: string; description: string }
...
export const STATUS_INDEX: Record<VisitStatus, number> = { queued: 0, processing: 1, ready: 2, completed: 3, cancelled: 4 }

export function normalizePlate(input: string) { return input.toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/^(\d{2})([A-Z]{1,3})(\d{2,5})$/, '$1 $2 $3') }
export function plateToSlug(plate: string) { return plate.replace(/\s+/g, '').toUpperCase() }
...
export function buildWhatsAppLink(phone: string | undefined | null, plate: string, businessSlug: string, origin: string) {
  ...
  return `https://wa.me/${intl}?text=${encodeURIComponent('Merhaba! Aracınız işleme alındı. Canlı takip linkiniz: ' + url)}`
}